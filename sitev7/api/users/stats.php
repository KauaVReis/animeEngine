...
$cacheKey = 'user_stats:' . $usuario_id;
$cachedStats = cacheGet($cacheKey);
if ($cachedStats !== null) {
    mysqli_close($conn);
...
$sql = "SELECT tipo_lista, COUNT(*) as count FROM listas_anime WHERE usuario_id = ? GROUP BY tipo_lista";
...
$sql = "SELECT SUM(progresso) as total FROM listas_anime WHERE usuario_id = ? AND tipo_lista = 'watching'";
$result = dbSelect($conn, $sql, 'i', [$usuario_id]);
...
$sql = "SELECT SUM(ac.episodios) as total 
        ...
        WHERE la.usuario_id = ? AND la.tipo_lista = 'completed'";
$result = dbSelect($conn, $sql, 'i', [$usuario_id]);
$row = mysqli_fetch_assoc($result);
...
// Favoritos
$sql = "SELECT COUNT(*) as count FROM listas_anime WHERE usuario_id = ? AND favorito = 1";
$result = dbSelect($conn, $sql, 'i', [$usuario_id]);
$row = mysqli_fetch_assoc($result);
...
// Conquistas
$sql = "SELECT COUNT(*) as count FROM conquistas WHERE usuario_id = ?";
$result = dbSelect($conn, $sql, 'i', [$usuario_id]);
$row = mysqli_fetch_assoc($result);
$stats['achievements'] = intval($row['count']);

// Dados do usuário
$sql = "SELECT username, xp, nivel FROM usuarios WHERE id = ?";
$result = dbSelect($conn, $sql, 'i', [$usuario_id]);
